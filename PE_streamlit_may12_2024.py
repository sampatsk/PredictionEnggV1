...
class TargetTaskRecommendation:
    ...
    def task_selection(_self, df, target_columns, th_val=30):
        ...
        """
        if len(df)<50:
            st.warning("The uploaded dataset has less than 50 records, therefore ML task type prediction might not be fully accurate. Please verify.")
    ...
    def column_selection(_self, df, input_column_names, col_map_invalid_chars):
        ...
        # if response code is other than 200 (success), prompt manual input for target column
        if not curl_execution.stdout.startswith('HTTP/2 200'):
            st.warning("LLM endpoint and key pairs could not be validated, please select the target column manually.") 
            origcollist = list(col_map_invalid_chars.values())
            return origcollist, None, ""
        # if response code is 200 (success), extract necessary information from response to recommend target column
            ...
            try:
                ...
                # reinitialize for the next run and return
                st.session_state.expltg = ""
                return input_column_names, None, ""
    ...
    def column_selection_task_selection(_self, csv_dataset): 
        ...
        with st.spinner('Identifying Target Column and ML Task Type...'):
            ...
            # if there are more than 10000 columns in the dataset, user has to manually select target column and ML task
            if len(colnames)>10000:
                st.warning("The dataset has over 10000 columns columns, please select the target column and ML task manually.")
                recommended_target_columns = st.multiselect("Target Columns", colnames, key="tc")
                recommended_ml_task = st.selectbox("What is the ML task?", tasklist, index=0, key="tt")
            # if LLM endpoint and api key are not configured properly, user has to manually select target column
            elif 'AZURE_OPENAI_ENDPOINT' not in os.environ or 'AZURE_OPENAI_API_KEY' not in os.environ:    
                st.warning("Environment variables AZURE_OPENAI_ENDPOINT or AZURE_OPENAI_API_KEY are not set up, please select the target column manually.")
